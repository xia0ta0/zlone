...
class DAO 
{
    ...
    private function processSQL()
    {
        ...
        if($this->mode == 'magic')
        {
            ...
            if($this->table == '')  triggerError('Must set the table name', __FILE__, __LINE__, $exit = true);
    ...
    public function query()
    {
        ...
        {
            triggerError($e->getMessage() . "<p>The sql is: $sql</p>", __FILE__, __LINE__, $exit = true);
        }
    }
    ...
    public function check($fieldName, $funcName)
    {
        ...
        if($funcName == 'unique')
        {
            ...
            try
            {
                 ...
                 if($row->count != 0) $this->logError($funcName, $fieldName, $fieldName, array($value));
            }
            catch (PDOException $e) 
            {
                triggerError($e->getMessage() . "<p>The sql is: $sql</p>", __FILE__, __LINE__, $exit = true);
        ...
        else
        {
            ...
            {
                $this->logError($funcName, $fieldName, $fieldName, $funcArgs);
            }
        }
     ...
     * @return void
     */
    public function logError($checkType, $fieldName, $fieldName, $funcArgs = array())
    {
        $error    = 'type error';
        $replaces = array_merge(array($fieldName), $funcArgs);     // the replace values.
